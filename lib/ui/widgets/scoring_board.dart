...
class ScoringBoard extends StatelessWidget {
  ...
  Widget _buildScoringPieChart(double circleSize) {
    final List<PieChartData> chartData = scoring.scoringCounts
        .map(
          (scoringCount) => PieChartData(
            scoringName[scoringCount.scoring],
            scoringCount.count,
            historyColor[scoringCount.scoring],
          ),
        )
        .toList();
          ...
          SfCircularChart(
            ...
            series: <CircularSeries>[
              DoughnutSeries<PieChartData, String>(
                ...
                dataLabelMapper: (PieChartData data, _) => data.x.toString(),
                dataLabelSettings: DataLabelSettings(
                  isVisible: true,
                  textStyle: TextStyle(fontSize: 6.0),
                ),
                // Radius of pie
                radius: '95%',
              ),
  ...
  Widget buildAreaChart() {
    ...
    color.add(Colors.blue.shade500);
    color.add(Colors.blue.shade400);
    color.add(Colors.blue.shade300);
    color.add(Colors.blue.shade200);
    color.add(Colors.blue.shade100);
    ...
    return Container(
      ...
      child: SfCartesianChart(
        ...
        margin: EdgeInsets.only(top: 5.0, left: 25.0, bottom: 15.0, right: 7.0),
  ...
  Widget _buildRetentionPieChart(double circleSize) {
    final List<PieChartData> chartData = scoring.retentionCounts
        ...
        .entries
        .map(
          (retentionCount) => PieChartData(
            retentionCount.value.label,
            retentionCount.value.count,
            historyColor[retentionCount.key],
          ),
        )
        .toList();
        ...
        children: [
          ...
          Center(
            child: Column(
              mainAxisSize: MainAxisSize.min,
              children: <Widget>[
                Text("平均定着度", style: _headerStyle),
                Text(
                  scoring.avg_retention.toString(),
                  style: TextStyle(
                    fontSize: 16.0,
                    fontWeight: FontWeight.w500,
                    color: Colors.white,
